...
export default function SearchBySeasonAndLeague({ setPlayers }) {
    ...
    async function handleSearch() {
        let res = await fetch(
            `https://topscorers.herokuapp.com/topscorer/year/${season}?league=${league}`
        );
        let data = await res.json();
        setPlayers(data.payload);
    }
    ...
    function handleClear() {
      ...
      return (
        <div style={{display: 'flex', justifyContent: 'center', margin: '0 0 1rem 0', gap: '1rem'}}>
          ...
          <button style={{color: 'white', fontSize: '1.1em', backgroundColor: 'green', borderRadius: '0.5em'}} onClick = {()=> {handleSearch()}}>Search</button>
          <button style={{color: 'white', fontSize: '1.1em', backgroundColor: 'red', borderRadius: '0.5em'}} onClick = {()=> {handleClear()}}>Clear</button>
        </div>
      );
    }